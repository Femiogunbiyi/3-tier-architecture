...
apt-get install -y docker.io netcat-openbsd dnsutils unzip curl
...
while [ $attempt -le $max_dns_attempts ]; do
    ...
    break
        ...
        log "WARNING: Failed to resolve database host after $max_dns_attempts attempts. Proceeding anymay..."
...
while [ $attempt -le $max_db_attempts]; do
    ...
    if nc -z -w 3 "$DB_HOST" "$DB_PORT" 2>/dev/null; then
        log "Successfully connected to the database at $DB_HOST:$DB_PORT"
        break
    else
        if [ $attempt -eq $max_db_attempts]; then
            log "WARNING: Could not verify database connectivity after $max_db_attempts attempts. Proceeding anyway..."
        else
            log "Database not yet reachable. Wiating 10 seconds before retry..."
            sleep 10
        fi
        attempt=$((attempt+1))
    fi
...
docker run -d \
    --name 3-tier-architecture-backend \
...
if docker ps | grep -q 3-tier-architecture-backend; then
    log "Backend container is running"
else
    log "Error: Backend container failed to start"
    docker logs 3-tier-architecture-backend
    exit 1
fi
...
for i in {1..30}; do
    if curl -s http://localhost:8080/3-tier-architecture > /dev/null 2>&1; then
        log "Backend is responding to requests"
        break
    fi
    if [ $i -eq 30 ]; then
        log "WARNING: Backend health check timeout"
        docker logs 3-tier-architecture-backend
    fi
    sleep 2
done
...
#!/bin/bash
response=$(curl -s -o /dev/null -w "%%{http_code}" http://localhost:8080/3-tier-architecture)
...
echo "*/5 * * * * /usr/local/bin/healthcheck.sh || systemctl restart docker && docker start 3-tier-architecture-backend" | crontab -

log "Backend setup completed successfully!"
log "Container logs: docker logs -f 3-tier-architecture-backend"
